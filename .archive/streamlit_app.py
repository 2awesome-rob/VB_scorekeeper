...
def over_net(check_for_point=True):
    ...
    if check_for_point: 
        if st.session_state.return_rally_result is None:
            st.session_state.return_rally_result = st.radio("They Rally", options=["Error", "Return"], key=f"result_{st.session_state.radio_counter}", index=None)
...
def handle_serve():
    ...
    with st.form(key=f"serve_form_{st.session_state.radio_counter}"):
        ...
        if submitted and serve_result:
            st.session_state.radio_counter += 1
            st.session_state.df.at[st.session_state.df.index[row_idx], "touch_serve"] = f"{player}:{serve_result}"
...
def rally():
    ...
    block_options = [st.session_state.df.iloc[row_idx][f"position_{pos(st.session_state.rotation + j)}"] for j in [3, 2, 1]]
    ...
    with st.form(key=f"rally_form_{st.session_state.radio_counter}"):
        if st.session_state.rally_step == 1:
            their_result = st.radio("Result of the serve:", options=["Over", "Error"], index=None)
        else:
            their_result = st.radio("Result of their rally:", options=["Over", "Error"], index=None)

        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.markdown("### Block")
            blocker = st.selectbox("Blocker", options=block_options, index=None)
            block_result = st.radio("Block Result", options=["Block", "Block:Kill", "Block:Tip", "Block:Assist", "NoBlock"], index=4)
            second_blocker = st.selectbox("Second Blocker", options=[p for p in block_options if p != blocker], index = None)
        with col2:
            st.markdown("### Touch 1")
            player1 = st.selectbox("Player", options=touch_options, key="player1_"+str(st.session_state.select_counter), index=None)
            touch1 = st.selectbox("Touch", options=["Dig", "Pass", "Set", "Attack"], key="touch1_"+str(st.session_state.select_counter), index=None)
            result1 = st.selectbox("Result", options=["Missed", "Error", "1", "2", "3", "Kill", "Over"], key="result1_"+str(st.session_state.select_counter), index=None)
        with col3:
            st.markdown("### Touch 2")
            player2 = st.selectbox("Player", options=touch_options, key="player2_"+str(st.session_state.select_counter), index=None)
            touch2 = st.selectbox("Touch", options=["Pass", "Set", "Attack"], key="touch2_"+str(st.session_state.select_counter), index=None)
            result2 = st.selectbox("Result", options=["Error", "1", "2", "3", "Kill", "Over"], key="result2_"+str(st.session_state.select_counter), index=None)
        with col4:
            st.markdown("### Touch 3")
            player3 = st.selectbox("Player", options=touch_options, key="player3_"+str(st.session_state.select_counter), index=None)
            touch3 = st.selectbox("Touch", options=["Pass", "Set", "Attack"], key="touch3_"+str(st.session_state.select_counter), index=None)
            result3 = st.selectbox("Result", options=["Error", "1", "2", "3", "Kill", "Over"], key="result3_"+str(st.session_state.select_counter), index=None)
        ...
        if submitted:
            ...
            if block_result != "NoBlock":
                st.session_state.df.at[st.session_state.df.index[row_idx], "touch_block"] = f"{blocker}:{block_result}"
            ...
            if result1:
                st.session_state.df.at[st.session_state.df.index[row_idx], "touch_1"] = f"{player1}:{touch1}:{result1}"
            if result2:
                st.session_state.df.at[st.session_state.df.index[row_idx], "touch_2"] = f"{player2}:{touch2}:{result2}"
            if result3:
                st.session_state.df.at[st.session_state.df.index[row_idx], "touch_3"] = f"{player3}:{touch3}:{result3}"
